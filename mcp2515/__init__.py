...
class MCP2515:  # pylint:disable=too-many-instance-attributes
    """A common shared-bus protocol.
        ...
        :param Literal crystal_freq: MCP2515 crystal frequency. Valid values are:\
            16000000, 10000000 and 8000000. Defaults to 8000000 (8MHz).\
    ...
    def __init__(
        ...
        *,
        ...
        crystal_freq: Literal[8_000_000, 10_000_000, 16_000_000] = 8_000_000,
        loopback: bool = False,
        silent: bool = False,
    ...
    def _read_rx_buffer(self, read_command):
        ...
        _tmp = bytearray(1)
        self._cs.value(0) 
        self._spi.write_readinto(bytes([read_command]), _tmp)
        self._buffer = bytearray(self._spi.read(15))
        self._cs.value(1) 
    ...
    def _write_message(self, tx_buffer, message_obj):
        ...
        self._cs.value(0) 
        self._spi.write(bytes([tx_buffer.LOAD_CMD]))
        self._spi.write(self._id_buffer)                            # write the 4 bytes of the ID: TXBnSIDH, TXBnSIDL, TXBnEID8, TXBnEID0
        self._spi.write(bytes([dlc]))                               # write a byte to the TXBnDLC register
        if isinstance(message_obj, Message):
            self._spi.write(message_obj.data)                   # write the data to the registers: TXBnD0-7
        self._cs.value(1)
    ...
    def _start_transmit(self, tx_buffer):
        ...
        self._cs.value(0) 
        self._spi.write(bytes([tx_buffer.SEND_CMD]))
        self._cs.value(1)
    ...
    def _write_id_to_register(self, register, can_id, extended=False):
        ...
        self._cs.value(0) 
        self._spi.write(bytes([_WRITE, register]))
        self._spi.write(self._id_buffer)
        self._cs.value(1)
    ...
    def _mod_register(self, register_addr, mask, new_value):
        ...
        self._cs.value(0) 
        self._spi.write(bytes([_BITMOD, register_addr, mask, new_value]))
        self._cs.value(1)
    ...
    def _read_register(self, regsiter_addr):
        ...
        self._cs.value(0) 
        self._spi.write(bytes([_READ, regsiter_addr]))
        _regVal = self._spi.read(1)
        self._cs.value(1)

        return int.from_bytes(_regVal, 'big')

    def _read_status(self):
        self._buffer[0] = _READ_STATUS
        self._cs.value(0) 
        self._spi.write(bytes([_READ_STATUS]))
        _regVal = self._spi.read(1)
        self._cs.value(1)
    ...
    def _set_register(self, regsiter_addr, register_value):
        self._cs.value(0) 
        self._spi.write(bytes([_WRITE, regsiter_addr, register_value]))
        self._cs.value(1)
